...
function SortableTaskCard({ task, deleteTask, updateTaskStatus, columnStatus }: SortableTaskProps) {
        ...
        {isAuthenticated && (
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button 
              ...
              className="p-1 text-[#C7C7CC] hover:text-[#FF3B30] pointer-events-auto"
            >
...
function BoardColumn({ column, tasks, deleteTask, updateTaskStatus }: BoardColumnProps) {
...
export default function BoardView() {
  ...
  const columns: { title: string; status: Task['status']; color: string }[] = [
    { title: 'Pending', status: 'pending', color: 'bg-slate-50/30' },
    { title: 'In Progress', status: 'in-progress', color: 'bg-slate-100/20' },
